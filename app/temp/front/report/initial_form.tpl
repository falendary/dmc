...
<div id="wrapper">
	...
	<header id="header" class="inner-new one-column-header">
        <div class="one-column-header__status">
            <strong class="status grey-status">первичное донесение</strong>
        </div>
        <div class="arrow-button">
            <a href="/"><span></span></a>
            {if !empty($REPORT)}<span>ЧС: {$REPORT.esID|default:$ESID}, ИД донесения: {$REPORT.reportHID}</span>{/if}
        </div>
	...
	<!-- header  end
		 main  start -->
	<div id="main">
		{if !empty($REPORT) || !empty($NEW_REPORT)}
		<div class="holder-form">
            <form method="post" id="initial-form" action="" autocomplete="off" class="form {if !empty($EDITABLE) && $EDITABLE}enabled{else}disabled{/if}{if !empty($LOCKED)} locked{/if}{if !empty($NEW_REPORT)} add{/if}">
				{if !empty($REPORT.initialReportID)}<input type='hidden' name='initialReportID' value='{$REPORT.initialReportID}' />{/if}
				<fieldset>
						...
						<div class="wrap-form">
							...
							<div class="double">
								...
								<div class="box-hide open">
									<div class="box">
										<label for="locationComment"><span>Комментарий</span></label>
										<div class="hold">
					...
					<div class="info">
						...
						<div class="holder-medicine">
							{foreach from=$MEDICAL_TEAMS item=MEDICAL_TEAM}
								<div class="medicine">
									<label for='medicalTeam_{$MEDICAL_TEAM.medicalTeamID}' {if !empty($MEDICAL_TEAM.hint)}data-title='{$MEDICAL_TEAM.hint}'{/if}>{$MEDICAL_TEAM.name}</label>
									...
									<div class="hold">
										<input type="number" min="0" placeholder="-" class="numeric" name='medicalTeams[{$MEDICAL_TEAM.medicalTeamID}][number]' id='medicalTeam_{$MEDICAL_TEAM.medicalTeamID}' value='{$REPORT.medicalTeams[$MEDICAL_TEAM.medicalTeamID].number|default:0}' {if empty($EDITABLE)}disabled='disabled'{/if} />
										<textarea cols="10" rows="30" placeholder="-" name='medicalTeams[{$MEDICAL_TEAM.medicalTeamID}][comment]' {if empty($EDITABLE)}disabled='disabled'{/if}>{$REPORT.medicalTeams[$MEDICAL_TEAM.medicalTeamID].comment|default}</textarea>
									</div>
								</div>
							{/foreach}
							...
							<li>
								<div class="top">
									<div class="bottom">
										...
										<span class="info2{if empty($REPORT) && !empty($EDITABLE)} generated{/if}">{if !empty($REPORT)}{$REPORT.createdUser.post}{elseif $EDITABLE}{$USER->info.post}{/if}</span>
							...
							<li>
								<div class="top">
									<div class="bottom">
										...
										<span class="info2{if empty($REPORT) && !empty($EDITABLE)} generated{/if}">{if !empty($REPORT.createdUser)}{$REPORT.createdUser.lastName} {$REPORT.createdUser.firstName|mb_truncate:1:''}.{$REPORT.createdUser.middleName|mb_truncate:1:''}.{elseif $EDITABLE}{$USER->info.lastName} {$USER->info.firstName|mb_truncate:1:''}.{$USER->info.middleName|mb_truncate:1:''}.{/if}</span>
                            ...
                            <li>
								<div class="top">
									<div class="bottom">
										...
										<span class="info2{if empty($REPORT.createdSigned) && !empty($CREATED_SIGNER)} generated{/if}">{if !empty($REPORT.createdSignedTimestamp)}{$REPORT.createdSignedTimestamp|date_format:$DATE_FORMAT}{elseif !empty($CREATED_SIGNER)}<проставляется при сохранении>{/if}</span>
					...
					<div class="footer in-page">
						<div class="holder-nav-new">
							<ul class="list">
								...
								{if !empty($REPORT.esID)}
									<li>
										ЧС: {$REPORT.esID}, ИД донесения: {$REPORT.reportHID}
									</li>
								{/if}
							</ul>
                                                        <!-- Закоментировано убрал строку. (empty($REPORT) || (!empty($REPORT.inStats))  -->
                            <div class="es-display one-column-report">
                                <label for="inStats">ЧС отображается в статистике</label>
                                <input type="checkbox" name="inStats" id="inStats" value="on" {if $inStats}checked="checked"{/if} {if empty($EDITABLE)}disabled='disabled'{/if}/>
                            </div>
                                                        
							{if empty($LOCKED)}
                                {if !empty($NEW_REPORT)}
                                    <button class="btn" type="submit" name="initialSubmit" value='doSave'>создать{if !empty($CHECKING)} и проверить{/if}</button>
								{elseif !empty($CHECKING)}
									<button class="btn" type="submit" name="initialSubmit" value='doCheck'>проверить</button>
                                {elseif !empty($SIGNING)}
                                    <button class="btn" type="submit" name="initialSubmit" value='doSign'>утвердить</button>
                                {elseif !empty($EDITABLE)}
                                    <button class="btn" type="submit" name="initialSubmit" value='doSave'>сохранить</button>
								{/if}
                                {if !empty($SUBSEQUENT_ENABLED)}
                                    <div style="clear:both; padding-top: 20px">
                                        <button class="btn" style='width: 298px; background-position: 253px 7px' type="submit" name="initialSubmit" value='doSubsequent'>создать последующее</button>
                                    </div>
                                {/if}
                                {if !empty($CAN_DELETE)}
                                    <div style="clear:both; padding-top: 20px">
									...
									<ul class="list-history">
										<li>
                                            ...
                                            <span class="t4">
                                                ...
                                                {/if}
                                                <a class="next" href="/final/{$ES_HISTORY.final.finalReportID}/"></a>
                                            </span>
										</li>
									</ul>
								</div>
										...
										{foreach from=$ES_HISTORY.subsequent item=REP}
											<li>
                                                ...
                                                <span class="t4">
                                                    ...
                                                    {/if}
                                                    <a class="text" href="/subsequent/{$REP.subsequentReportID}/"></a>
                                                </span>
											</li>
										{/foreach}
									</ul>
		...
		{else}
			<h1>Ошибка</h1>
			<p>Первичное донесение не найдено</p>
		{/if}
	</div>
